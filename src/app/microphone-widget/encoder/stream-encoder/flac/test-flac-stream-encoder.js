 ...
 *
 *    $ node test-flac-stream-encoder PCM_IN_FILE FLAC_OUT_FILE
 */
...
const fs = require('fs');
const { FlacStreamEncoder } = require('./flac-stream-encoder.js');
const encoder = new FlacStreamEncoder({ onEncode : onEncode,  blockSize : 1024, sampleRate : 48000 });

const pcmInFile = args[2].startsWith('/')? args[2] : currentFolder+args[2];
const flacOutFile = args[3].startsWith('/')? args[3] : currentFolder+args[3];

const inFileStream = fs.createReadStream(pcmInFile, {start: 44, highWaterMark: 2048});
const outFileStream = fs.createWriteStream(flacOutFile);
